...
class SemVerStrategySpec extends Specification {
    ...
    def 'infer returns correct version'() {
        ...
        expect:
        strategy.doInfer(project, gitBuildService, locator) == new ReleaseVersion('1.2.3-beta.1+abc123', '1.2.2', createTag)
        where:
    ...
    def 'infer fails if stage is not listed in stages property'() {
        ...
        when:
        strategy.doInfer(project, gitBuildService, null)
        then:
    ...
    def 'infer fails if precedence enforced and violated'() {
        ...
        when:
        strategy.doInfer(project, gitBuildService, locator)
        then:
        thrown(GradleException)
    }
    ...
    private def mockRepoClean(boolean isClean) {
        (0..2) * gitBuildService.isCleanStatus() >> isClean
    }

    private def mockCurrentBranch() {
        (0..1) * gitBuildService.currentBranch >> 'refs/heads/master'
        (0..1) * gitBuildService.head >> 'refs/heads/master'
    }

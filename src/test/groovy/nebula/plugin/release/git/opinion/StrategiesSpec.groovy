...
class StrategiesSpec extends Specification {
    ...
    def 'BuildMetadata.COMMIT_ABBREVIATED_ID uses current HEAD\'s abbreviated id'() {
        given:
        def commit = new Commit(id: '5e9b2a1e98b5670a90a9ed382a35f0d706d5736c', abbreviatedId: '5e9b2a1')
        def initialState = new SemVerStrategyState(currentHead: commit)

        expect:
        def inferred = Strategies.BuildMetadata.COMMIT_ABBREVIATED_ID.infer(initialState)
        def expected = initialState.copyWith(inferredBuildMetadata: '5e9b2a1')
